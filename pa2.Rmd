...
We will extract only columns of interest for our analysis -- begin date, event type, and data columns related to fatalities, injuries, property damage, and crop damage, and damage modifiers.
...
To help begin our exploratory analysis, we convert the beginning date `BGN_DATE` in the data to a numerical year value and investigate the number of unique event types (`EVTYPE`) in the data by year. In particular, we're looking for years in which more than three event types were recorded.
...
There appears to be a discrepancy here in which; `EVTYPE` diversity seems to begin in 1993 rather than 1996. The NOAA website now refers to a more recent version of the database, and it's possible that different decisions have been made in the meantime. Let us confirm both that only Tornado, Thunderstorm Wind, and Hail data are available before 1993, and also that a wide range of event types are available from 1993 onward.
...
```{r explore_1993}
  ...
  dt.explore[order(-records)][1:10]
```  
A breadth of `EVTYPE` categories is present in both 1993-1995 and 1996-1998, with no compelling justification present for excluding the former years. With that in mind, we select **1993 to 2011** as the period for this analysis. 
...
We take a conservative approach and, i n the absence of specific information to the contrary, will regard **+**, **-**, and **?** to indicate same-order-of-magnitude uncertainty. Additionally, given the small number of records involved and the extremely large sums of cumulative damage in USD over this timeframe, we feel the risk of having introduced large-scale errors with this decision is extremely minimal. 

We may now calculate true property and crop damage values to aid in data summary.
...
We will now obtain cumulative damage measures by summing over all 1993-2011 records in the data by provided event type. This paves the way for our exploratory analysis of `EVTYPE`.
...
#### Note on Winter Ambiguity

In general, the lines of distinction between varying categories of winter weather events were the most frequent and ambiguous in the dataset. 

In the final analysis, we will see that this does not make a significant impact to the top-ranked outiers for damage-causing events. However, we will also discuss a view of major winter categories as a whole.
...
```{r process_data_final}
  ...
  !is.na(unique(dt.sum$event))
```  
Note that there are three "Other" event types included in `dt.results`. If all 48 NWS events produced at least one damage-causing record in the data, then `dt.results` will contain 51 events.

With the `EVTYPE` so coerced in the new `event` variable, we are now ready to present results.

## Results

We represent our results in panel plots, one for personal health impact and one for economic impact. 
...
```{r results_plot_personal, fig.width = 10, fig.height = 10, fig.cap = "Figure 1: Results (Personal Health)", results = "asis"}
...
Tornados are primarily responsible for recording personal health damage in the United States. 

After tornadoes, a quartet of event groups each contribute sizably and similarly to the total. A larger number of miscellaneous event types fill in the remainder. Overall, the causes of personal health damage in the U.S. are quite diverse.

1. Tornadoes
1. Thunderstorms, Heat-related events, Flooding, and Winter-related events
1. A number of miscellaneous event types with lower-order impact
...
1. Storm Surges
1. Tornadoes, Storms (winds and rains), Winter events, Hail, Drought, and Wildfire (all on a much smaller scale)
...
These are defined and explained in NWS Instruction 10-1605. As represented in our data, we have capitalized every letter in `event` names. All 48 distinct event types produced at least one damage-causing record from 1993 to 2011.
...
Our results were represented visually in the report. The full data is printed here as a detailed reference. The `DT` package is used to present the `data.table` output in readable, interactive format.
...
```{r appendix_full_data, results = "asis"}
  ...
  dt.results.summary <- rbind(
    list(
      ...
      Property = sum(dt.results$prop),
      Crop = sum(dt.results$crop),
  ...
  # Use DT library to print in interactive format.
  DT::datatable(dt.results.summary,
    options = list(pageLength = 52),
    class = 'cell-border stripe',
    caption = 
      "Table 1: Personal health and economic damage (USD) from NOAA Storm Events Database, 1993-2011"
  ) %>% 
    formatCurrency(c("Fatalities", "Injuries", "Total"), currency = "") %>%
    formatCurrency(c("Property", "Crop", "Total Damage"))
